...
class SmartGrid:
    ...
    def __init__(
        ...
        self.config = SmartGridConfigModel(
            ...
            inverter_keepalive_per_period_wh=100 / 1000,
        ...
        self.rates: list[RatesModel] = []

    def main(self) -> dict:

        current_battery_level: float = 0

        try:
            current_battery_level = self.get_battery_level()
        except BatteryLevelUnobtainableException as err:
            _LOGGER.error(err)
        ...
        for schedule in sorted_schedules[:trim]:
            periods = [str(s.hour).zfill(2) + ":" + str(s.minute).zfill(2) for s in schedule.force_charge]
            periods.sort()
            s_list.append({
    ...
    def calculate_schedule(
        ...
        force_charge_values: list[datetime] = []
        ...
        for start in start_values:
            ...
            soc_start = self.convert_battery_level_to_soc(
                battery_capacity_wh=config.battery_capacity_wh,
                battery_level_wh=start_battery_level
            )

            if force_charge:
                grid = self.force_charge_result(
                    battery_level=start_battery_level + solar - loads,
                    max_force_charge=config.maximum_charge_per_period_wh,
                    max_battery_level=config.battery_capacity_wh
                )
                force_charge_values.append(start)

            end_battery_level = self.charge_battery_result(
                battery_level=start_battery_level,
                add_level=solar + grid - loads,
                max_battery_level=config.battery_capacity_wh,
                min_battery_level=config.minimum_battery_level_wh
            )
            ...
            if not force_charge:
                ...
                grid = self.pull_grid_result(
                    ...
                    loads=loads,
                    min_battery_level=config.minimum_battery_level_wh,
                    inverter_keepalive_per_period_wh=config.inverter_keepalive_per_period_wh
                )

            soc_end = self.convert_battery_level_to_soc(
                battery_capacity_wh=self.config.battery_capacity_wh,
                battery_level_wh=end_battery_level
            )
        ...
        return SmartGridDataSchedule(
            ...
            force_charge=force_charge_values,
            grid=grid_values,
            iteration=iteration,
            loads=loads_values,
            rates=rates_values,
    ...
    def pull_grid_result(
        ...
        :param inverter_keepalive_per_period_wh: think this has to do with the inverter always pulling a
        ...
        return inverter_keepalive_per_period_wh
    ...
    def get_battery_level(self) -> float:
        """
        Battery level is calculated by getting the SoC reading from FoxCloud
        and converting it to Wh based on the battery capacity
        """
        _LOGGER.info(f"Querying {self.battery_soc_sensor}")
        soc = float(self.hass.states.get(self.battery_soc_sensor).state)
        _LOGGER.info(f"Obtained SoC value of {str(soc)} {type(soc)}")
        battery_level = self.convert_soc_to_battery_level_wh(
            battery_capacity_wh=self.config.battery_capacity_wh,
            soc=soc
        )
        _LOGGER.info(f"Battery level is {battery_level} kWh")
        return battery_level

    @staticmethod
    def convert_soc_to_battery_level_wh(battery_capacity_wh: float, soc: float) -> float:
        soc = soc * (battery_capacity_wh / 100)
        return soc

    @staticmethod
    def convert_battery_level_to_soc(battery_capacity_wh: float, battery_level_wh: float) -> int:
        soc = (100 / battery_capacity_wh) * battery_level_wh
        return int(soc)


class BatteryLevelUnobtainableException(Exception):
